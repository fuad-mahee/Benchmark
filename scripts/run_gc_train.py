...
def main():
    ...
    save_jsonl(build_examples(tok, train_ids), out / "train.jsonl")
    save_jsonl(build_examples(tok, heldout_ids), out / "heldout.jsonl")
    ...
    examples = build_examples(tok, train_ids)
    with Timer() as t:
        train(model, tok, examples, mcfg, gc, out / "adapter")

    save_json(run_metadata(model=args.model, config=gc, n_train=len(train_ids),
